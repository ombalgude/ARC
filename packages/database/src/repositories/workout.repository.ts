...
import { exerciseLogs, workoutDays, workoutExercises, workoutPlans, workoutSessions } from "../schema/index.js";
...
export type CreateWorkoutExerciseInput = Omit<NewWorkoutExercise, "id" | "notes"> &
...
export async function createPlan(input: CreateWorkoutPlanInput, executor: DatabaseExecutor = db) {
...
export async function createDay(input: CreateWorkoutDayInput, executor: DatabaseExecutor = db) {
...
export async function createWorkoutExercise(input: CreateWorkoutExerciseInput, executor: DatabaseExecutor = db) {
  const [exercise] = await executor.insert(workoutExercises).values(input).returning();
  if (!exercise) {
